...
public class ProjectDownloadServiceImpl implements ProjectDownloadService {
    ...
    public void downloadProjectAsZip(String projectPath, String downloadFileName, HttpServletResponse response) {
        ...
        ThrowUtils.throwIf(!rootDir.exists() || !rootDir.isDirectory(), ErrorCode.NOT_FOUND_ERROR, "项目目录不存在");
        ...
        // 定义文件过滤器
        FileFilter filter = file -> isPathAllowed(rootDir.toPath(), file.toPath());
        try {
            // 打包下载
            ZipUtil.zip(response.getOutputStream(), StandardCharsets.UTF_8, false, filter, rootDir);
